...
readonly SETTINGS_CONF_PATH="$SCRIPT_PARENT_DIR/../config/settings.conf"
...
function setup_local_vars_and_dirs() {
  ...
  touch "$LOCAL_PREVIOUS_MANIFEST_LOCATION"
  touch "$LOCAL_JOB_LOG_PATH"
  touch "$LOCAL_CRAWLER_OUTPUT_FILE_PATH"
  touch "$LOCAL_NEW_MANIFEST_PATH"
