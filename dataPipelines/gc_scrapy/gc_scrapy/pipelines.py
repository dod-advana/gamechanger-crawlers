...
class FileDownloadPipeline(MediaPipeline):
    ...
    def open_spider(self, spider):
        ...
        self.previous_manifest_path = Path(spider.previous_manifest_location)

        if (
            spider.dont_filter_previous_hashes
            or not self.previous_manifest_path.exists()
        ):
            self.previous_hashes = set()
        else:
            self.load_hashes_from_cumulative_manifest(spider.name)

    def load_hashes_from_cumulative_manifest(self, spider_name):

        logger.debug("Reading in previous manifest")
        count = 0
        with self.previous_manifest_path.open(mode="r") as f:
            for line in f.readlines():
                count += 1
                if count % 1000 == 0:
                    logger.debug(f"{count} lines read in")
                if not line.strip():
                    continue
    ...
    @staticmethod
    def get_first_supported_downloadable_item(
        downloadable_items: list,
    ) -> Union[dict, None]:
        ...
        return next(
            (
                ...
                if item["doc_type"] in SUPPORTED_FILE_EXTENSIONS
            ),
            None,
        )
            ...
            try:
                if info.spider.download_request_headers:
                    yield scrapy.Request(
                        url, headers=info.spider.download_request_headers, meta=meta
                    )
                else:
    ...
    def add_to_manifest(self, item):
        ...
        with self.job_manifest_path.open("a") as f:
            try:
                f.write(
                    json.dumps(
                        {
                ...
                logger.error(
                    f"Failed to write to manifest file {self.job_manifest_path.resolve()}: {e}"
                )
        ...
        for (_, (okay, response, reason)) in results:
            if not okay:
                ...
                if compression_type:
                    ...
                    ).with_suffix(f".{compression_type}")
