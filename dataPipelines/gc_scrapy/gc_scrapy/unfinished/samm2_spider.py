...
class SammSpider(GCSpider):
    ...
    def parse(self, response):
        if 'https://samm.dsca.mil/policy-memoranda/PolicyMemoList-All' in response.url:
            ...
            for row in response.xpath('//div[@class="view-content"]//table/tbody/tr'):
                ...
                pm_status = pm_status_text.strip() if pm_status_text is not None else "" # Grab column to evaluate, exclude whitespace
                if pm_status != "Incorporated": # Excluding Incorporated 
                    relative_urls = row.xpath('td[2]/a/@href').get() # Grab link to pdf container
                    
                    absolute_url = urljoin(base_url, relative_urls)
                    yield scrapy.Request(absolute_url, callback=self.parse_document_page) # link to pull from


        elif 'https://samm.dsca.mil/listing/chapters' in response.url:
            base_url = "https://samm.dsca.mil"
    ...
    def parse_document_page(self, response):
            if response.url.startswith('https://samm.dsca.mil/policy-memoranda/PolicyMemoList-All'):
            ...
            elif response.url.startswith('https://samm.dsca.mil/listing/chapters'):
