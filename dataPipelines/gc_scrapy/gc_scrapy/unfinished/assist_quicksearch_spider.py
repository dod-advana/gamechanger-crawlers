...
class AssistQuicksearchSpider(GCSeleniumSpider):
    ...
    def parse(self, response: TextResponse):
        ...
        search_btn: WebElement = driver.find_element_by_css_selector(
            "#GetFilteredButton"
        )
        search_btn.click()
        ...
        while True:
            ...
            for doc_row in doc_rows:
                ...
                if has_img == "N":  # no available images
                    continue
                doc_link = doc_row.css(
                    'td:nth-child(2) > a[title="Go to the Document Details."]::attr(href)'
                ).get()
    ...
    def parse_doc_details(self, response: TextResponse):
        ...
        for doc_row in doc_rows:
            href = doc_row.css(
                'td:nth-child(1) a[title="Click here to view the Document Image"]::attr(href)'
            ).get()
            ...
            if not doc_date.strip():
                ...
                publication_date = datetime.strptime(doc_date, "%d-%b-%Y").strftime(
                    "%Y-%m-%d"
                )
            ...
            doc_token = re.match(
                r"javascript:spawnPDFWindow\('\.\/ImageRedirector\.aspx\?token=.+,(?P<token>\d+)\);",
                href,
            )["token"]
    ...
    def construct_doc_num(self, general_id, spec_sheet, part_description):
        ...
        part_description_match = re.match(
            part_description_regex, part_description
        ).groupdict()
        num = general_id
