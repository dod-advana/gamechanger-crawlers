...
class SammSpider(GCSpider):
    ...
    def parse(self, response):
        if 'PolicyMemoList-All' in response.url:
            self.parse_policy_memoranda(response)
        else:
            self.parse_samm_chapters(response)

    def parse_policy_memoranda(self, response):
        ...
        for row in response.xpath('//div[@class="view-content"]//table/tbody/tr'):
            ...
            if pm_status != "Incorporated": 
                ...
                yield scrapy.Request(
                    absolute_url, 
                    callback=self.parse_memoranda_page, 
                    cb_kwargs={'doc_title': doc_title, 'publication_date': pub_date}
                )

    def parse_memoranda_page(self, response, doc_title, publication_date):
    ...
    def parse_samm_chapters(self, response):
        base_url = "https://samm.dsca.mil"
        ...
        for accordion in accordions:
            ...
            for relative_url in relative_urls:
                ...
                yield scrapy.Request(absolute_url, callback=self.parse_samm_chapters_page)

    def parse_samm_chapters_page(self, response):
