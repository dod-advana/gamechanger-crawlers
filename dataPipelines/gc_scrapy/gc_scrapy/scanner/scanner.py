...
def parse_args():
    ...
    parser.add_argument(
        "-c", "--cores", help="num of cores for scanner to use, default is max", type=int, default=mp.cpu_count())
    parser.add_argument("--scanner-path", help="path of dlp scanner",
                        type=str, default="/srv/dlp-scanner/dlp-scanner.sh")
    parser.add_argument(
        "--input-path", help="directory or file to scan", type=str)

    return parser.parse_args()


def run_scanner(input_path: t.Union[str, Path]) -> bool:
    """
    ...
    try:
        ...
        script_path = Path(
            os.path.dirname(os.path.realpath(__file__)), 'dlp-scanner.sh'
        ).resolve()
        ...
        print('PROCESS:::', completed_process.returncode)
...
def process_dir(script_path: t.Union[str, Path], input_path: t.Union[str, Path], cores: int):
    ...
    with ProcessPoolExecutor(max_workers=cores) as pp:
        results = pp.map(_starmap_run_scanner, ((script_path, p)
                                                for p in input_path.iterdir() if p.is_file() and p.name != 'manifest.json'))
        for r in results:
            report_progress(r)

    # process manifest json last to signify upload is over
    if Path(input_path, 'manifest.json').is_file():
        manifest_result = run_scanner(
            script_path, Path(input_path, 'manifest.json'))
