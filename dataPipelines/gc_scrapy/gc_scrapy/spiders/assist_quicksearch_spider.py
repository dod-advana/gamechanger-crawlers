...
class DoDSpider(GCSeleniumSpider):
        ...
        while True:
            ...
            for doc_row in doc_rows:
                ...
                if has_img != 'Y':  # no available images
                    continue
                doc_num = doc_row.css('td:nth-child(2)::text').get()  # XXX:
                # doc_num = ''.join(doc_row.css('td:nth-child(2) *::text').getall())  # XXX: delete
    ...
    def parse_doc_details(self, response: TextResponse):
        ...
        if not doc_url:  # no publicly available download
            return
        doc_url = re.match(r"javascript:spawnPDFWindow\('(?P<url>.+)',.+\);", doc_url)['url']
        web_url = urljoin(response.url, doc_url)

        # FIXME: how to actually get download URL?
    ...
    def clean_name(self, name):
        return ' '.join(re.sub(r'[^a-zA-Z0-9. ()-]', '', self.ascii_clean(name).replace('/', '_')).split())
