...
class HASCSpider(GCSpider):
    ...
    def get_next_relative_url(response):
        return response.css(
            "div#copy-content div.navbar-inner a:nth-child(3):not(.disabled)::attr(href)"
        ).get()
    ...
    def parse_hearings_table_page(self, response):
        ...
        rows = response.css("div#copy-content div.recordsContainer table tbody tr")
    ...
    def parse_hearing_detail_page(self, response):
        try:
            ...
            header = response.css("div#copy-content div.header")
            ...
            permalink = response.css(
                "div#copy-content div.foot p.permalink a::attr(href)"
            ).get()

            downloadable_items = [
                {"doc_type": "html", "web_url": permalink, "compression_type": None}
            ]
