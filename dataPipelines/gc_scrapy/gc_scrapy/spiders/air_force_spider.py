...
class AirForcePubsSpider(GCSeleniumSpider):
    ...
    def parse(self, response):
        ...
        Select(
            driver.find_element_by_css_selector(self.item_count_dropdown_selector)
        ).select_by_value("100")
        ...
        while has_next_page:
            try:
                el = driver.find_element_by_css_selector(anchor_after_current_selector)
    ...
    def parse_table(self, driver):
        ...
        for row in webpage.css(row_selector):
            ...
            certification_date_raw = row.css("td:nth-child(4) span::text").get(
                default=""
            )
            ...
            if prod_num.find("CFETP") != -1:
                ...
                doc_type = (
                    type_extract[0]
                    if type_extract
                    else type_pattern_start.findall(prod_num)[0]
                )
                doc_num = (
                    re.sub(doc_type, "", prod_num_new)
                    if type_extract
                    else re.sub(doc_type, "", prod_num)
                )
                doc_name = " ".join((doc_type, doc_num))
            else:
            ...
            cac_login_required = (
                True
                if any(x in url_raw for x in self.cac_required_options)
