...
class BupersSpider(GCSpider):
    ...
    def clean(text):
        return (
            text.replace("\xa0", " ").encode("ascii", "ignore").decode("ascii").strip()
        )
    ...
    def parse(self, response):
        ...
        for row in rows:
            ...
            doc_title = " ".join(
                self.filter_empty([self.clean(text) for text in doc_titles_raw])
            )
            dates_cleaned = self.filter_empty([self.clean(text) for text in dates_raw])
            links_cleaned = self.filter_empty(links_raw)

            # happy path, equal num of docs, links, dates
            if (
                (len(doc_nums_cleaned) == len(links_cleaned) == len(dates_cleaned))
                ...
                for i in range(len(doc_nums_cleaned)):
                    ...
                    doc_name = self.match_old_doc_name(f"{self.doc_type} {doc_num}")
            ...
            # doc num was split, combine them into one string
                ...
                len(doc_nums_cleaned) > len(dates_cleaned)
                and len(links_cleaned) == len(dates_cleaned)
            ) or (
                any(
                    item in ["Vol 1", "Vol 2", "CH-1", "w/CH-1"]
                    for item in doc_nums_cleaned
                )
            ):
            ...
            # there are supplemental downloadable items
                ...
                raise Exception("Row data not captured, doesnt match known cases", row)
