...
class SASCSpider(GCSpider):
    ...
    def get_last_page_number(response) -> str:
        last_page_str = response.css(
            "#main_column > div.pagination-right.pull-left > div > select > option:last-child::text"
        ).get()
    ...
    def follow_pdf_redirect(self, response):
        redirect_doc_href = response.css("#content div.row p a::attr(href)").get()

        downloadable_items = [
            {"doc_type": "pdf", "web_url": redirect_doc_href, "compression_type": None}
        ]
    ...
    def parse_hearing_detail_page(self, response):
        try:
            ...
            title = self.join_multitext(main.css("h1:nth-child(1) *::text").getall())
            ...
            downloadable_items = [
                {"doc_type": "html", "web_url": response.url, "compression_type": None}
            ]
            ...
            for aside in asides:
                ...
                yield scrapy.Request(
                    aside_href,
                    callback=self.follow_pdf_redirect,
                    meta={"doc": aside_doc},
                )
