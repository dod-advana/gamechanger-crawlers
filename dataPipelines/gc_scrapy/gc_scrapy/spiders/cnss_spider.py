...
class CNSSSpider(GCSpider):
    ...
    start_urls = ["https://www.cnss.gov/CNSS/index.cfm"]
    ...
    def parse_page(self, response):
        split_func: t.Callable[[str], t.Tuple[str, str, str]] = response.meta[
            "split_func"
        ]
        ...
        for row in rows[1:]:
            ...
            doc_type_num_raw = row.css(
                'td:nth-child(2) p.documentTitle span[itemprop="name"]::text'
            ).get()
            doc_title_raw = row.css(
                'td:nth-child(2) p.documentTitle span[itemprop="description"]::text'
            ).get()

            publication_date_raw = row.css(
                'td:nth-child(2) p.documentInfo span[itemprop="dateCreated"]::text'
            ).get()
