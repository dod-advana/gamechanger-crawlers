...
class FmrSpider(GCSpider):
    ...
    start_urls = ["https://comptroller.defense.gov/FMR/vol1_chapters.aspx"]
    ...
    def parse(self, response):
        ...
        for link in volume_links:
            ...
            yield response.follow(
                url=link, callback=self.parse_volume, meta={"vol_num": vol_num}
            )
    ...
    def parse_volume(self, response):
        ...
        for row in rows:
            ...
            doc_title_raw = "".join(row.css("td:nth-child(2) *::text").getall())
