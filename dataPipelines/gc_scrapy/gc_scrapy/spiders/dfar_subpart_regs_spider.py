...
class DfarsSubpartSpider(GCSeleniumSpider):
    ...
    start_urls = ["https://www.acquisition.gov/dfars"]
    ...
    def parse(self, response: TextResponse):
        ...
        num_buttons = len(
            driver.find_elements_by_css_selector("a.agov-open-subparts-link")
        )
        for i in range(num_buttons):
            # the rows seem to be recreated so find the button again
            button = driver.find_elements_by_css_selector("a.agov-open-subparts-link")[
                i
            ]
    ...
    def parse_rows(self, rows: List[Selector], pub_date: str) -> Iterator[DocItem]:
        for row in rows:
            ...
            version_hash_fields = {"item_currency": href_raw, "pub_date": pub_date}
