...
class DHASpider(GCSpider):
    ...
    def parse(self, response):
        ...
        pdf_urls = [
            f"https://www.health.mil{href}"
            for href in rows.css("a::attr(href)").getall()
        ]
        publication_dates = rows.css("td.fd-col2::text").getall()

        for (doc_data, publication_date, pdf_url) in zip(
            docs_data, publication_dates, pdf_urls
        ):
