...
class LegislationSpider(GCSpider):
    ...
    start_urls = ["https://www.govinfo.gov/wssearch/rb/bills?fetchChildrenOnly=0"]
    ...
    def get_api_detail_url(package_id: str) -> str:
        return (
            f"https://www.govinfo.gov/wssearch/getContentDetail?packageId={package_id}"
        )
    ...
    def parse(self, response):
        ...
        yield response.follow(
            url=specific_congress_url,
            callback=self.get_bill_type_data,
            meta={"congress_num": congress_num},
        )
    ...
    def get_bill_num_chunks(self, response):
        ...
        for bill_num_chunk_path in bill_num_chunks:
            ...
            yield response.follow(
                url=bill_num_chunk_url,
                callback=self.get_package_ids,
                meta={"offset": 0},
            )
    ...
    def parse_detail_data(self, response):
        ...
        congress_num_str = detail_data.get("Congress Number").replace(" Congress", "")

        bill_type_raw, _, doc_num = detail_data.get("Bill Number").rpartition(" ")
