    ...
    def parse(self, response: TextResponse):
        ...
        while True:
            try:
                ...
                doc_rows: list = driver.find_elements_by_class_name(
                    "items.alist-more-here > *"
                )
            ...
            time.sleep(
                2
            )  # wait between pages to disencourage getting banned. adds ~16 minutes to runtime
            for doc_row in doc_rows[1:]:
                try:
                    doc_type = "MARADMIN"
                    doc_title = doc_row.find_element_by_class_name(
                        "msg-title.msg-col a"
                    ).get_attribute("textContent")
                    doc_num = doc_row.find_element_by_class_name(
                        "msg-num.msg-col a"
                    ).get_attribute("textContent")
                    publication_date = doc_row.find_element_by_class_name(
                        "msg-pub-date.msg-col"
                    ).get_attribute("textContent")
                    web_url = doc_row.find_element_by_class_name(
                        "msg-title.msg-col a"
                    ).get_attribute("href")
                    doc_name = (
                        doc_type + " " + doc_num.replace("/", "-") + " " + doc_title
                    )
                    ...
                    doc_item = DocItem(
                        doc_name=" ".join(
                            self.ascii_clean(doc_name).split(" ")[:8]
                        ).replace("/", "-"),
            ...
            try:
                ...
                next_btn: WebElement = driver.find_element_by_css_selector(
                    "a.fas.fa.fa-angle-right.da_next_pager"
                )
                try:
