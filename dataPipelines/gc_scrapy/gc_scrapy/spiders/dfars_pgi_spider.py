...
class DoDSpider(GCSpider):
    ...
    def parse(self, response: TextResponse):
        ...
        publication_date = next(
            ...
            for match in map(
                lambda o: re.match(
                    r"Current Version \((?P<date>\d{2}/\d{2}/\d{4})\)", o.get()
                ),
                options,
            )
            if match
        ...
        yield response.follow(
            table_iframe_src, callback=self.parse_table_iframe, meta=meta
        )
    ...
    def parse_table_iframe(self, response: TextResponse):
        ...
        for row in rows:
            if row.attrib["class"] == "rule":
                ...
                part_title = (
                    part_and_title_split[1]
                    if len(part_and_title_split) > 1
                    else part_num
                )
                prev_num = part_num
                ...
                if doc_subpart in ("TABLE OF CONTENTS", "COVER PAGE") or (
                    doc_subpart.startswith("PART") and prev_num.startswith("APPENDIX")
                ):
    ...
    def clean_name(self, name):
        return " ".join(
            re.sub(
                r"[^a-zA-Z0-9. ()\\-]", "", self.ascii_clean(name).replace("/", "-")
            ).split()
        )
