...
class CFRSpider(GCSpider):
    ...
    start_urls = ["https://www.govinfo.gov/wssearch/rb/cfr?fetchChildrenOnly=0"]
    ...
    def parse(self, start_url_response):
        ...
        years = [
            node.get("nodeValue").get("browsePath")
            for node in data.get("childNodes", [])
        ]

        for year in years:
            year_url = (
                f"https://www.govinfo.gov/wssearch/rb//cfr/{year}/?fetchChildrenOnly=1"
            )
            yield start_url_response.follow(
                url=year_url, callback=self.handle_title_nums
            )
        ...
        for title_num_dict in title_num_nodes:
            ...
            if title_num_dict.get("volumes"):
                for vol in title_num_dict.get("volumes"):
                    ...
                    vol_data.update({"packageid": package_id, "volume": vol_num})
