...
class TRADOCSpider(GCSpider):
    ...
    start_urls = ["https://www.mynavyhr.navy.mil/References/Messages/"]
    ...
    def parse_page(self, response: TextResponse):
        ...
        for table_row in table_rows:
            ...
            doc_title = self.join_text(
                table_row.css("td:nth-child(2) *::text").getall()
            )

            is_revoked = "cancelled" in doc_title.lower()

            # there seem to be some dead hidden links to the BUPERS site so ignore
            doc_url = table_row.css(
                'td:nth-child(2) a:not([href*="/bupers-npc/"])::attr(href)'
            ).get()
            ...
            downloadable_items = [
                {"doc_type": "txt", "web_url": web_url, "compression_type": None}
            ]
