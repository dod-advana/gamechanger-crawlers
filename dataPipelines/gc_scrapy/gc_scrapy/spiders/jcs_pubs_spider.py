...
class JcsPubsSpider(GCSpider):
    ...
    cac_required_options = ["CAC", "PKI certificate required", "placeholder", "FOUO"]

    def parse(self, response):
        doc_links = [
            a
            for a in response.css("div.librarylinkscontainer a")
            if "CJCS" in a.css("::attr(href)").get()
        ]
    ...
    def parse_doc_table_page(self, response):
        ...
        for row in rows:
            ...
            cac_login_required = (
                True
                if any(x in href_raw for x in self.cac_required_options)
        ...
        try:
            ...
            next_page_link = next(
                a
                for a in nav_table.css("a.CommandButton")
                if a.css("::text").get() == "Next"
            )
            yield response.follow(
                url=next_page_link, callback=self.parse_doc_table_page
            )
        except Exception:
            pass
