...
class TRADOCSpider(GCSpider):
    ...
    start_urls = ["https://adminpubs.tradoc.army.mil/index.html"]
    rotate_user_agent = True

    _doc_num_rgx = re.compile(
        r"^(?P<num>[-0-9a-zA-Z]+)?(?: with )?(?:Change (?P<change>\d+))?$",
        re.IGNORECASE,
    )
    ...
    def parse_page(self, response: TextResponse):
        ...
        doc_category_code = re.match(r"TRADOC .+ \((?P<code>.+)s\)", doc_category)[
            "code"
        ]
        ...
        for table_row in table_rows:
            ...
            if table_row.css(
                f"td:nth-child({title_col_i}) > span.fileLink > span.CACrequired"
            ):
                continue  # no download available here

            doc_nums = list(
                text.strip()
                for text in table_row.css(f"td:nth-child({num_col_i})::text").getall()
                if text.strip()
            )
            doc_dates = list(
                text.strip()
                for text in table_row.css(f"td:nth-child({date_col_i})::text").getall()
                if text.strip()
            )
            row_title = table_row.css(f"td:nth-child({title_col_i})::text").get()
            doc_url_lists = []
            file_span: Selector
            for file_span in table_row.css(
                f"td:nth-child({title_col_i}) > span.fileLink"
            ):
            ...
            for doc_num, doc_date, doc_url_list in zip(
                doc_nums, doc_dates, doc_url_lists
            ):
    ...
    def clean_name(self, name):
        return " ".join(
            re.sub(
                r"[^a-zA-Z0-9. ()-_]", "", self.ascii_clean(name).replace("/", "_")
            ).split()
        )
