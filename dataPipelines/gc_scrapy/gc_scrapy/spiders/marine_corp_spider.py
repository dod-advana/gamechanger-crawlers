...
def name_from_type_title(raw_data: dict) -> None:
    raw_data["doc_name"] = raw_data["doc_type_raw"] + ": " + raw_data["doc_title_raw"]


def set_all_transformations(raw_data: dict, transform_dict: dict) -> None:
    set_num_func = transform_dict.get("set_num_func")
    set_type_func = transform_dict.get("set_type_func")
    set_doc_name_func = transform_dict.get("set_doc_name_func")
...
class MarineCorpSpider(GCSpider):
    ...
    current_page = 1
    start_urls = [f"{base_url}{current_page}"]
    ...
    def parse(self, response):
        ...
        for row in rows:
            try:
                ...
                doc_type_raw = row.css("div.list-type span::text").get(default="")
                doc_type_num_raw = row.css("div.list-title::text").get(default="")
                doc_title_raw = row.css("div.cat span::text").get(default="")
                doc_status_raw = row.css("div.status::text").get(default="")
                ...
                version_hash_fields = {"status": doc_status_raw}
                ...
                cac_login_required = (
                    True
                    if any(x in doc_title for x in self.cac_required_options)
                    else False
                )
    ...
    def parse_download_page(self, response):
        ...
        href_raw = response.css("div.download-section a::attr(href)").get(default="")
        if not href_raw:
            href_raw = response.css("div.body-text a::attr(href)").get(default="")
        if not href_raw:
        ...
        doc_item["version_hash_raw_data"].update({"item_currency": href_raw})

        doc_type = self.get_href_file_extension(href_raw)
        web_url = self.url_encode_spaces(href_raw)

        doc_item["downloadable_items"] = [
            {"doc_type": doc_type, "web_url": web_url, "compression_type": None}
        ]
