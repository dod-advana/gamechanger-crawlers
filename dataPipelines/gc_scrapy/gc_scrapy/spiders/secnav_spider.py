...
class SecNavSpider(GCSpider):
    ...
    def start_rate_limited_yield(self):
        if self.had_error:
            ...
            print(f"ready to process: len of q: {len(self.q)}, done: {self.done}")
            while self.q:
                doc = self.q.pop(0)
                try:
                    sleep(0.75)
            ...
            print(
                "SecNavSpider queue still accumulating... no errors but not ready to process\n",
    ...
    def parse(self, response):
        try:
            ...
            for r in data["Row"]:
                ...
                cac_login_required = re.match("^[A-Za-z]", doc_num) is not None
