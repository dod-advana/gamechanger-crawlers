...
@pytest.mark.parametrize("spider_type,manifest_file", [("test_us_code_spider", "us_code_manifest.json")])
def test_spider_manifest_output(spider_type: str, manifest_file: str):
    ...
    true_manifest_data = list(read_semi_json_file(_TEST_SPIDER_RESOURCES / manifest_file))

    os.system(command)

    manifest_data = list(read_semi_json_file(_TEST_TMP_PATH / "manifest.json"))
